//! Generates `docs/earth_to_mars.svg` for this crate's README: a spacecraft
...
fn render_svg(
    ...
    mars_orbit_km: &[DVec3],
    trajectory_km: &[DVec3],
    earth_departure_km: DVec3,
    mars_arrival_km: DVec3,
) -> String {
    ...
    let _ = writeln!(
        ...
        "<!--\n  Terramenta - a heliocentric Earth-to-Mars trip: Spacecraft::update\n  \
         ...
         captured, with no jump in position at the hand-off.\n\n  Generated by the \
         `earth_to_mars` example in this crate (see its own README); do not edit by hand.\n-->"
    );
    let _ = writeln!(
        svg,
    ...
    let _ = writeln!(
        svg,
        r##"<circle cx="{earth_x:.2}" cy="{earth_y:.2}" r="5" fill="#78aaff"/>"##
    );
    let _ = writeln!(
        svg,
        r##"<text x="{:.2}" y="{:.2}" fill="#cfe0ff" font-size="12">Earth, departure</text>"##,
        earth_x + 9.0,
        earth_y - 9.0
    );

    let (mars_x, mars_y) = to_point(mars_arrival_km);
    let _ = writeln!(
        svg,
        r##"<circle cx="{mars_x:.2}" cy="{mars_y:.2}" r="5" fill="#ff8c5a"/>"##
    );
    let _ = writeln!(
        svg,
        r##"<text x="{:.2}" y="{:.2}" fill="#cfe0ff" font-size="12">Mars, arrival / capture</text>"##,
        mars_x + 9.0,
        mars_y - 9.0
    );
    ...
    let _ = writeln!(
        svg,
        r##"<text x="{:.2}" y="{:.2}" fill="rgba(207,224,255,0.55)" font-size="11" text-anchor="middle">Yellow: spacecraft trajectory · blue ring: Earth's orbit · orange ring: Mars' orbit</text>"##,
        WIDTH / 2.0,
        HEIGHT - 14.0
    );
